...
export const createUserCliente = async (data: INewClientFormFields): Promise<any> => {
  const access_token = localStorage.getItem('access_token');
  if(!accessTokenUserExists()) return  Promise.reject('No hay un token de acceso');
  try {
...
export const createCommonUser = async (data: IUserRegistrationFormValues):Promise<any> => {
  const access_token = localStorage.getItem('access_token');
  if(!accessTokenUserExists()) return  Promise.reject('No hay un token de acceso');
  try {
    const response = await instance.post('auth/create-user',data, {
